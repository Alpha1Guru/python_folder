...
def verifloat(decimal): #fuction verifies for correct float or decimal numbers
    failed_result = check_unwanted(decimal) #checks the text in the check_unwanted func
    while True in failed_result :  #while the text failed the test
        if decimal == "":
        ...
        failed_result = check_unwanted(decimal) #checks the text again untill correct
    return decimal #returns the correct(ed) text

def verint(integer): #function verifies for correct integers only!
    failed_result = check_unwanted(integer,float =False) #checks the text as an integer and not a float
    while True in failed_result :  #while the text failed the test
        if integer == "":
        ...
        failed_result = check_unwanted(integer,float=False) #checks the text again untill correct
